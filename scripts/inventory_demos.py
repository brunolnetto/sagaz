...
def get_demo_metadata(demo_dir: Path) -> Optional[Dict]:
...
def list_demos(domain_filter: Optional[str] = None) -> Dict[str, Dict]:
...
def report_markdown(demos: Dict[str, Dict]) -> str:
    ...
    total_tested = sum(
        1 for patterns in demos.values()
...
def report_table(demos: Dict[str, Dict]) -> str:
    ...
    for domain in sorted(demos.keys()):
        ...
        for pattern in sorted(patterns.keys()):
            ...
            lines.append(
                f"| {domain} | {pattern} | {status} | {desc[:60]} |"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate demo module inventory report"
    )
    parser.add_argument(
        "--json",
        action="store_true",
