...
from sagaz.exceptions import SagaExecutionError
from sagaz.types import ParallelFailureStrategy, SagaStatus, SagaStepStatus
...
class TestSetupSagaLogging:
    ...
    def test_setup_with_no_json_format(self):
        ...
        from sagaz.monitoring.logging import SagaLogger, setup_saga_logging

        logger = setup_saga_logging(log_level="DEBUG", json_format=False, include_console=True)
    ...
    def test_setup_with_no_console(self):
        ...
        logger = setup_saga_logging(log_level="INFO", json_format=True, include_console=False)
...
class TestOrchestratorCountRolledBack:
    ...
    async def test_count_rolled_back(self):
        ...
        await saga.add_step(
            name="succeeding_step", action=lambda ctx: "success", compensation=compensation
        )

        await saga.add_step(name="failing_step", action=failing_action)
...
class TestStateMachineTypeChecking:
    ...
    def test_state_machine_imports(self):
        ...
        from sagaz.state_machine import get_valid_next_states, validate_state_transition
...
class TestSagaStepStateFromDict:
    ...
    def test_from_dict_without_optional_fields(self):
        ...
        minimal_dict = {"name": "minimal_step", "status": "pending"}
...
class TestInMemoryStorageEdgeCases:
    ...
    async def test_cleanup_with_various_statuses(self):
        ...
        for saga_id, status in statuses:
            await storage.save_saga_state(
                saga_id=saga_id, saga_name="Test", status=status, steps=[], context={}
            )
