...
class TestCopyDockerComposeFiles:
    def test_dev_mode(self, tmp_path):
        ...
        os.chdir(tmp_path)
        try:
            with patch("sagaz.cli.init.local._create_inmemory_docker_compose") as m:
    ...
    def test_with_ha(self):
        ...
        with (
            patch("sagaz.cli.init.utils.copy_resource") as mock_res,
            patch("sagaz.cli.init.utils.copy_dir_resource") as _mock_dir,
        ):
    ...
    def test_no_ha_no_dev(self):
        ...
        with patch("sagaz.cli.init.utils.copy_resource") as mock_res:
...
class TestInitDockerCompose:
    ...
    def test_copies_when_no_existing_file(self, tmp_path):
        ...
        os.chdir(tmp_path)
        try:
            with patch("sagaz.cli.init.local._copy_docker_compose_files") as mock_copy:
                _init_docker_compose("redis", False)
    ...
    def test_overwrites_on_confirm(self, tmp_path):
        ...
        try:
            ...
            with (
                patch("sagaz.cli.init.utils.click") as mock_click,
                patch("sagaz.cli.init.local._copy_docker_compose_files") as mock_copy,
            ):
...
class TestInitMonitoring:
    def test_with_ha(self):
        ...
        with patch("sagaz.cli.init.utils.copy_dir_resource") as mock_dir:
...
class TestLogLocalInitStart:
    def test_dev_mode_log(self):
        ...
        with patch("sagaz.cli.init.utils.console", mock_console):
    ...
    def test_no_console(self):
        ...
        with patch("sagaz.cli.init.utils.console", None):
            # Should not raise
            _log_local_init_start("redis", False, "postgresql", dev_mode=False)
...
class TestInitLocal:
    def test_calls_all_steps(self, tmp_path):
        ...
        try:
            with (
                patch("sagaz.cli.init.local._log_local_init_start") as m_start,
                patch("sagaz.cli.init.local._init_docker_compose") as m_compose,
                patch("sagaz.cli.init.local._init_monitoring") as m_mon,
                patch("sagaz.cli.init.local._log_local_init_complete") as m_complete,
            ):
    ...
    def test_no_monitoring_when_disabled(self, tmp_path):
        ...
        try:
            with (
                patch("sagaz.cli.init.local._log_local_init_start"),
                patch("sagaz.cli.init.local._init_docker_compose"),
                patch("sagaz.cli.init.local._init_monitoring") as m_mon,
                patch("sagaz.cli.init.local._log_local_init_complete"),
            ):
...
class TestInitSelfhost:
    def test_basic(self, tmp_path):
        ...
        try:
            mock_console = MagicMock()
            with (
                patch("sagaz.cli.init.utils.console", mock_console),
                patch("sagaz.cli.init.utils.click") as mock_click,
            ):
                mock_click.echo = _noop
    ...
    def test_with_observability(self, tmp_path):
        ...
        try:
            with (
                patch("sagaz.cli.init.utils.console", MagicMock()),
                patch("sagaz.cli.init.utils.click") as mock_click,
            ):
                mock_click.echo = _noop
    ...
    def test_no_console(self, tmp_path):
        ...
        try:
            with (
                patch("sagaz.cli.init.utils.console", None),
                patch("sagaz.cli.init.utils.click") as mock_click,
            ):
                mock_click.echo = _noop
                _init_selfhost("redis", with_observability=False)
...
class TestCopyK8sManifests:
    ...
    def test_postgresql_with_ha(self, tmp_path):
        ...
        try:
            ...
            with (
                patch("sagaz.cli.init.utils.copy_resource") as mock_res,
                patch("sagaz.cli.init.utils.copy_dir_resource"),
                patch("sagaz.cli.init.utils.click") as mock_click,
            ):
                mock_click.echo = _noop
...
class TestInitK8s:
    def test_basic(self, tmp_path):
        ...
        try:
            with (
                patch("sagaz.cli.init.k8s._log_k8s_init_start"),
                patch("sagaz.cli.init.k8s._prepare_k8s_directory", return_value=True),
                patch("sagaz.cli.init.k8s._copy_k8s_manifests"),
                patch("sagaz.cli.init.k8s._copy_k8s_observability"),
                patch("sagaz.cli.init.k8s._copy_k8s_benchmarks"),
                patch("sagaz.cli.init.k8s._log_k8s_init_complete") as m_complete,
            ):
    ...
    def test_aborts_when_prepare_returns_false(self, tmp_path):
        ...
        try:
            with (
                patch("sagaz.cli.init.k8s._log_k8s_init_start"),
                patch("sagaz.cli.init.k8s._prepare_k8s_directory", return_value=False),
                patch("sagaz.cli.init.k8s._copy_k8s_manifests") as m_copy,
            ):
                _init_k8s(with_observability=True, with_benchmarks=False)
    ...
    def test_handles_exception(self, tmp_path):
        ...
        try:
            with (
                ...
                patch("sagaz.cli.init.k8s._prepare_k8s_directory", return_value=True),
                patch(
                    "sagaz.cli.init.k8s._copy_k8s_manifests",
                ...
                # Should not raise; exception is swallowed and sagaz.cli.init.utils.click.echo called
        finally:
            os.chdir(old_cwd)
...
class TestCopyDirResource:
    ...
    def test_not_found_returns_silently(self):
        ...
        with patch("sagaz.cli.init.utils.pkg_resources") as mock_pkg:
            mock_traversable = MagicMock()
...
class TestConsoleImportError:
    ...
    def test_importerror_branch_executed_on_reimport(self):
        ...
        mods_to_remove = ["sagaz.cli._init_handlers", "sagaz.cli.init.utils"]
...
class TestRemainingBranches:
    def test_selfhost_unknown_broker_skips_broker_env(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            with (
                patch("sagaz.cli.init.utils.console", None),
                patch("sagaz.cli.init.utils.click.echo"),
            ):
