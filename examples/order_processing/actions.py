...
async def check_availability(ctx: Any) -> Dict[str, Any]:
    ...
    return {"available": True, "stock_count": 100, "sku": ctx.get("sku", "DEFAULT-SKU")}
...
class PaymentProvider:
    ...
    async def charge_card(
        cls, card_token: str, amount: float, currency: str = "USD", metadata: Optional[Dict] = None
    ) -> Dict[str, Any]:
    ...
    async def refund(cls, transaction_id: str, amount: float) -> Dict[str, Any]:
        ...
        return {"refund_id": f"ref_{transaction_id}", "amount": amount, "status": "refunded"}
...
async def capture_payment(ctx: Any) -> Dict[str, Any]:
    ...
    return {"transaction_id": f"TXN-{auth_id}", "status": "captured"}
...
async def validate_payment_method(ctx: Any) -> Dict[str, Any]:
    ...
    if method not in valid_methods:
        return {"valid": False, "error": f"Unsupported payment method: {method}"}
...
async def send_confirmation_email(ctx: Any) -> Dict[str, Any]:
    ...
    return {"email_sent": True, "email_id": f"EMAIL-{ctx.order_id}"}
