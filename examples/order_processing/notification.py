...
class NotificationService:
    ...
    async def send_email(
        ...
        if random.random() < cls._email_failure_rate:
            ...
            return {"status": "failed", "to": to, "subject": subject, "error": "Simulated failure"}
    ...
    async def send_sms(cls, phone: str, message: str) -> Dict[str, Any]:
        ...
        if random.random() < cls._sms_failure_rate:
            ...
            return {"status": "failed", "phone": phone, "error": "Simulated failure"}
    ...
    @classmethod
    async def send_push(
        cls, user_id: str, title: str, body: str, data: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        if random.random() < cls._push_failure_rate:
            ...
            return {"status": "failed", "user_id": user_id, "error": "Simulated failure"}
...
async def send_payment_receipt_email(ctx: Any) -> Dict[str, Any]:
    ...
    result = await NotificationService.send_email(
        ...
        template_data={"transaction_id": ctx.get("transaction_id"), "amount": ctx.get("amount", 0)},
    )
...
async def send_shipping_notification(ctx: Any) -> Dict[str, Any]:
    ...
    logger.info(
        f"Sending shipment notification for order {ctx.order_id if hasattr(ctx, 'order_id') else 'unknown'}"
    )
    ...
    email_result = await NotificationService.send_email(
        ...
        else f"user-{ctx.user_id if hasattr(ctx, 'user_id') else 'unknown'}@example.com",
