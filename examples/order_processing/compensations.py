...
async def release_inventory(ctx: Any) -> Dict[str, Any]:
    ...
    logger.info(
        f"Releasing inventory for order {ctx.order_id if hasattr(ctx, 'order_id') else 'unknown'}"
    )
    ...
    if hasattr(ctx, "reserved_items"):
        for item in ctx.reserved_items:
            released_items.append(
                {"sku": item["sku"], "quantity": item["quantity"], "released": True}
            )
...
async def refund_payment(ctx: Any) -> Dict[str, Any]:
    ...
    transaction_id = (
        ctx.get("transaction_id") if hasattr(ctx, "get") else getattr(ctx, "transaction_id", None)
    )
...
async def void_authorization(ctx: Any) -> Dict[str, Any]:
    ...
    return {"voided": True, "authorization_id": auth_id}
...
async def reverse_wallet_payment(ctx: Any) -> Dict[str, Any]:
    ...
    return {"reversed": True, "transaction_id": transaction_id}
...
async def release_payment_hold(ctx: Any) -> Dict[str, Any]:
    ...
    return {"released": True, "hold_id": hold_id}
...
async def reverse_payment_fee(ctx: Any) -> Dict[str, Any]:
    ...
    return {"reversed": True, "fee_id": fee_id}
...
async def restore_payment_credits(ctx: Any) -> Dict[str, Any]:
    ...
    return {"restored": True, "credits": credits}
...
async def send_order_cancellation_notification(ctx: Any) -> Dict[str, Any]:
    ...
    logger.info(
        f"Sending cancellation notification for order {ctx.order_id if hasattr(ctx, 'order_id') else 'unknown'}"
    )
    await asyncio.sleep(0.05)

    return {"notification_sent": True, "type": "cancellation"}
...
async def send_payment_failure_notification(ctx: Any) -> Dict[str, Any]:
    ...
    return {"notification_sent": True, "type": "payment_failure"}
...
async def retract_bulk_notifications(ctx: Any) -> Dict[str, Any]:
    ...
    return {"retracted": True, "count": len(successful)}
...
async def suppress_notification_preferences(ctx: Any) -> Dict[str, Any]:
    ...
    return {"suppressed": True, "user_id": user_id}
...
async def cancel_shipment(ctx: Any) -> Dict[str, Any]:
    """Cancel shipment"""
    logger.info(
        f"Canceling shipment for order {ctx.order_id if hasattr(ctx, 'order_id') else 'unknown'}"
    )
    await asyncio.sleep(0.1)  # Simulate shipment cancellation

    cancel_data = {"cancelled": True, "status": "cancelled"}
...
async def send_cancellation_email(ctx: Any) -> Dict[str, Any]:
    ...
    logger.info(
        f"Sending cancellation email for order {ctx.order_id if hasattr(ctx, 'order_id') else 'unknown'}"
    )
