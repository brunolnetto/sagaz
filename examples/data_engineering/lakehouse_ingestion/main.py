...
class LakehouseIngestionSaga(Saga):
    ...
    async def ingest_to_bronze(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(f"🥉 BRONZE: Ingesting raw data")
        ...
        if random.random() < 0.03:  # 3% failure rate
            raise SagaStepError(f"Source unavailable: {source_path}")
    ...
    async def delete_bronze_partition(self, ctx: dict[str, Any]) -> None:
        ...
        if bronze_path:
            logger.warning(f"🧹 BRONZE: Deleting partition")
    ...
    async def process_to_silver(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(f"🥈 SILVER: Processing and cleaning data")
    ...
    async def delete_silver_partition(self, ctx: dict[str, Any]) -> None:
        ...
        if silver_path:
            logger.warning(f"🧹 SILVER: Deleting partition")
    ...
    async def aggregate_to_gold(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(f"🥇 GOLD: Aggregating for analytics")
        ...
        if random.random() < 0.05:  # 5% failure rate
            raise SagaStepError(
                f"Gold aggregation failed: Insufficient memory for aggregation. "
    ...
    async def delete_gold_partition(self, ctx: dict[str, Any]) -> None:
        ...
        if gold_path:
            logger.warning(f"🧹 GOLD: Deleting partition")
    ...
    async def update_data_catalog(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        # Register partitions
        for layer, table in [("bronze", bronze_table), ("silver", silver_table), ("gold", gold_table)]:
...
async def successful_lakehouse_demo():
    ...
    print("\n" + "=" * 80)
    print("🏠 Lakehouse Ingestion Saga - Medallion Architecture Demo")
    print("=" * 80)
    print("    Bronze (Raw) → Silver (Clean) → Gold (Aggregated)")
    print("=" * 80)
    ...
    print(f"\n{'✅' if result.get('catalog_updated') else '❌'} Lakehouse Ingestion Result:")
    print(f"   Ingestion ID:   {result.get('ingestion_id')}")
    print(f"\n   Layer Summary:")
    print(f"   ├─ Bronze:      {result.get('bronze_record_count', 0):,} records (raw)")
    print(f"   ├─ Silver:      {result.get('silver_record_count', 0):,} records ({result.get('quality_ratio', 0):.1%} retained)")
    print(f"   └─ Gold:        {result.get('gold_record_count', 0):,} records (aggregated)")
    print(f"\n   Data Quality:")
    print(f"   ├─ Duplicates removed: {result.get('duplicates_removed', 0):,}")
    print(f"   ├─ Nulls filled:       {result.get('nulls_filled', 0):,}")
    print(f"   └─ Invalid removed:    {result.get('invalid_removed', 0):,}")


async def failed_lakehouse_demo():
    """Demonstrate lakehouse ingestion failure with layer-by-layer rollback."""
    print("\n" + "=" * 80)
    print("⚠️  Lakehouse Ingestion Saga - Failure with Layer Rollback Demo")
    print("=" * 80)
    print("(Note: Gold aggregation has 5% failure rate)\n")
    ...
    for attempt in range(5):
        try:
            ...
            print(f"\n✅ Attempt {attempt + 1} succeeded: {result.get('ingestion_id')}")
        except SagaStepError as e:
            print(f"\n❌ Attempt {attempt + 1} failed: {e}")
            print("\n✅ Layer-by-layer rollback completed:")
            print("   🥇 Gold partition deleted")
            print("   🥈 Silver partition deleted")
            print("   🥉 Bronze partition deleted")
            print("   📚 Catalog entries removed")
            print("   ➡️  All layers consistent (no orphaned data)")
            break
...
async def main():
    ...
    print("\n" + "=" * 80)
    print("📚 Lakehouse Ingestion Demo Complete")
    print("=" * 80)
    print("\nMedallion Architecture Layers:")
    print("  🥉 BRONZE: Raw data, no transformations")
    print("  🥈 SILVER: Cleaned, deduplicated, validated")
    print("  🥇 GOLD:   Aggregated, optimized for analytics")
    print("\nKey Benefits of Sagaz for Lakehouse:")
    print("  ✅ Layer-by-layer rollback on failure")
    print("  ✅ No orphaned data in any layer")
    print("  ✅ Data lineage tracking")
    print("  ✅ Quality metrics at each layer")
    print("  ✅ Downstream notification")
