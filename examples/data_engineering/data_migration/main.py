...
class DataMigrationSaga(Saga):
    ...
    async def cleanup_export_files(self, ctx: dict[str, Any]) -> None:
        ...
        if export_results:
            ...
            for table, result in export_results.items():
    ...
    async def transform_schema(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(f"✅ Schema transformation complete")
    ...
    async def cleanup_transformed_files(self, ctx: dict[str, Any]) -> None:
        ...
        if transform_results:
            ...
            for table, result in transform_results.items():
    ...
    async def verify_row_counts(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        for table in export_results.keys():
...
async def successful_migration_demo():
    ...
    print("\n" + "=" * 80)
    print("🚀 Data Migration Saga - Successful Migration Demo")
    print("=" * 80)
    ...
    try:
        ...
        print(f"\n{'✅' if result.get('status') == 'SUCCESS' else '❌'} Migration Result:")
        print(f"   Migration ID:    {result.get('migration_id')}")
        print(f"   Source:          {result.get('source_system')}")
        print(f"   Target:          {result.get('target_system')}")
        print(f"   Tables:          {result.get('tables_migrated')}")
...
async def failed_migration_demo():
    ...
    print("\n" + "=" * 80)
    print("⚠️  Data Migration Saga - Failure with Rollback Demo")
    print("=" * 80)
    print("(Note: Checksum verification has 5% failure rate per table)\n")
    ...
    for attempt in range(3):
        try:
            ...
            print(f"\n✅ Attempt {attempt + 1} succeeded: {result.get('migration_id')}")
        except SagaStepError as e:
            ...
            print("   • Migration marked as FAILED")
            break
...
async def main():
    ...
    print("=" * 80)
    print("\nKey Benefits of Sagaz for Migrations:")
    print("  ✅ Atomic multi-table migration")
    print("  ✅ Automatic backup before migration")
    print("  ✅ Row count verification")
    print("  ✅ Checksum verification")
    print("  ✅ Full rollback on any failure")
    print("  ✅ Audit trail of migration steps")
