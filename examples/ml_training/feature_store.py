...
class FeatureStoreSaga(Saga):
    ...
    async def validate_features(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(f"✅ All feature validations passed")
    ...
    async def cleanup_validation_artifacts(self, ctx: dict[str, Any]) -> None:
        ...
        logger.warning(f"🧹 Cleaning up validation artifacts")
    ...
    async def publish_to_feature_store(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(f"Writing to online feature store...")
        await asyncio.sleep(0.2)
        
        # Write to offline store
        logger.info(f"Writing to offline feature store...")
        await asyncio.sleep(0.2)
        
        # Update metadata
        logger.info(f"Updating feature metadata catalog...")
        await asyncio.sleep(0.1)
        ...
        logger.info(f"✅ Features published successfully")
    ...
    async def rollback_feature_store(self, ctx: dict[str, Any]) -> None:
        ...
        - Invalidate caches
        """
        logger.warning(f"⏪ Rolling back feature store changes")
        ...
        if commit_id:
            ...
            if ctx.get("online_store_updated"):
                logger.info(f"Rolling back online store...")
                await asyncio.sleep(0.2)
            
            # Rollback offline store
            if ctx.get("offline_store_updated"):
                logger.info(f"Rolling back offline store...")
                await asyncio.sleep(0.2)
            
            # Revert metadata
            if ctx.get("metadata_updated"):
                logger.info(f"Reverting metadata catalog...")
                await asyncio.sleep(0.1)
            
            logger.info(f"✅ Rollback complete - feature store restored to previous state")
...
async def failed_pipeline_demo():
    ...
    try:
        ...
        print(f"   (Validation rules were lenient enough to pass)")
    except SagaStepError as e:
        print(f"\n❌ Pipeline failed as expected: {e}")
        print(f"✅ Automatic compensation completed successfully")
        print(f"   Staged data cleaned up")
        print(f"   Computed features removed")
        print(f"   Feature store not updated (consistency maintained)")
        print(f"   Ready for retry with adjusted validation rules")
