...
if TYPE_CHECKING:  # pragma: no cover
    from sagaz.storage.interfaces import SagaStorage, OutboxStorage
...
class StorageManager(BaseStorageManager):
    ...
    async def _initialize_unified(self, backend_type: str) -> None:
        ...
        if backend_type == "memory":
            ...
            raise ValueError(f"Unknown backend type: {backend_type}")  # pragma: no cover
    ...
    async def _create_saga_storage(self, backend_type: str, url: str | None) -> "SagaStorage":
        ...
        if backend_type == "memory":
        ...
        elif backend_type == "postgresql":  # pragma: no cover
            from sagaz.storage.backends.postgresql.saga import PostgreSQLSagaStorage
            storage = PostgreSQLSagaStorage(url)
            await storage.initialize()
            return storage
        
        elif backend_type == "redis":  # pragma: no cover
            from sagaz.storage.backends.redis.saga import RedisSagaStorage
            storage = RedisSagaStorage(url)
            await storage.initialize()
            return storage
        
        elif backend_type == "sqlite":  # pragma: no cover
            from sagaz.storage.backends.sqlite.saga import SQLiteSagaStorage
            ...
            raise ValueError(f"Unknown saga backend: {backend_type}")  # pragma: no cover
    ...
    async def _create_outbox_storage(self, backend_type: str, url: str | None) -> "OutboxStorage":
        ...
        if backend_type == "memory":
            ...
            raise ValueError(f"Unknown outbox backend: {backend_type}")  # pragma: no cover
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        if self._saga_storage:
            try:
                ...
                saga_health = {"status": "unhealthy", "error": str(e)}  # pragma: no cover
        ...
        if self._outbox_storage:
            try:
                ...
                outbox_health = {"status": "unhealthy", "error": str(e)}  # pragma: no cover
...
def create_storage_manager(
    ...
    **kwargs,
    ...
    Examples:
        # Same backend (shared connection pool)
        >>> manager = create_storage_manager("postgresql://localhost/db")
        >>> await manager.initialize()
        
        # Different backends (hybrid mode)
        >>> manager = create_storage_manager(
        ...     saga_url="postgresql://localhost/saga_db",
        ...     outbox_url="redis://localhost:6379",
        ... )
    ...
    if backend:
        ...
        if backend == "memory":
            ...
            if not url and not saga_url:
                raise ValueError("url is required for PostgreSQL backend")
            return StorageManager(url=url, saga_url=saga_url, outbox_url=outbox_url, **kwargs)
        elif backend == "redis":
            default_url = "redis://localhost:6379"
            return StorageManager(url=url or default_url, saga_url=saga_url, outbox_url=outbox_url, **kwargs)
        elif backend == "sqlite":
            return StorageManager(url=url or ":memory:", saga_url=saga_url, outbox_url=outbox_url, **kwargs)
        else:
            raise ValueError(f"Unknown backend: {backend}")
    
    # Check for hybrid mode (different URLs provided)
    ...
    if url is not None and url != "memory://" and url != "":
