...
def discover_examples(category: Optional[str] = None) -> dict[str, Path]:
    """
...
def list_examples_cmd(category: Optional[str] = None):
...
def interactive_cmd(category: Optional[str] = None):
...
def _examples_menu_loop(category: str) -> str:  # pragma: no cover
    ...
    while True:
        ...
        # Build example menu entries
        menu_entries = []
        for name, path in sorted_examples:
            desc = get_example_description(path)
            if len(desc) > 40:
                desc = desc[:37] + "..."
            # Show just the example name (without category prefix)
        ...
        # After execution, show "Press Enter to continue"
        if console:
            console.print("\n[dim]" + "─" * 60 + "[/dim]")
...
def _fallback_interactive_simple(category: Optional[str] = None):  # pragma: no cover
