...
class RealEstateSimulator:
    ...
    async def record_deed(
        ...
        if random.random() < 0.05:  # 5% chance of recording delay
            raise SagaStepError(f"County recorder system temporarily unavailable")
...
class PropertyClosingSaga(Saga):
    ...
    async def title_search(self, ctx: SagaContext) -> dict[str, Any]:
        ...
        if result["liens"]:
            raise SagaStepError(f"Title has liens: {result['liens']}")
    ...
    async def appraisal_review(self, ctx: SagaContext) -> dict[str, Any]:
        ...
        if result["status"] != "approved":
            raise SagaStepError("Appraisal did not meet requirements")
    ...
    async def final_walkthrough(self, ctx: SagaContext) -> dict[str, Any]:
        ...
        if result["issues_found"]:
            raise SagaStepError(f"Walkthrough issues: {result['issues_found']}")
...
async def main():
    ...
    print("=" * 80)
    print("🏠 Property Closing Saga Demo")
    print("=" * 80)
    print("\nThis example demonstrates multiple legal commitment pivot points.")
    print("Both escrow release and deed recording are points of no return.\n")
    
    saga = PropertyClosingSaga()
    
    # Scenario 1: Successful closing
    print("\n" + "=" * 80)
    print("🟢 Scenario 1: Successful Property Closing")
    print("=" * 80)
    ...
    print(f"\n{'✅' if result.get('saga_id') else '❌'} Closing Result:")
    print(f"   Saga ID: {result.get('saga_id')}")
    print(f"   Deed Recording #: {result.get('recording_number', 'N/A')}")
    print(f"   Wire Reference: {result.get('wire_reference', 'N/A')}")
    print(f"   Pivot 1 (Escrow): {result.get('pivot_1_reached', False)}")
    print(f"   Pivot 2 (Deed): {result.get('pivot_2_reached', False)}")
    print(f"   Keys Transferred: {result.get('keys_transferred', [])}")
    
    # Scenario 2: Pre-pivot failure
    print("\n" + "=" * 80)
    print("🟡 Scenario 2: Pre-Pivot Failure (Title Issue)")
    print("=" * 80)
    print("If title search reveals liens before closing:\n")
    print("  → Appraisal cancelled")
    print("  → Contingencies reinstated")
    print("  → No funds released (pivot not reached)")
    print("  → Transaction can be amended or cancelled")
    
    # Scenario 3: Between pivots
    print("\n" + "=" * 80)
    print("🔴 Scenario 3: Between Pivots (Escrow Released, Deed Fails)")
    print("=" * 80)
    print("""
The most critical scenario in real estate: escrow is released but
deed recording fails. In this case:

- Seller HAS the money (pivot 1 complete)
- Buyer DOES NOT have legal ownership (pivot 2 failed)

Forward recovery is CRITICAL:
1. RETRY - County system back up, record immediately
2. RETRY_WITH_ALTERNATE - Fix document issues, re-submit
3. MANUAL_INTERVENTION - Title company urgently resolves

This is why multiple pivots require careful handling - partial
completion can leave parties in legal limbo.
""")
