...
class ManufacturingProductionSaga(Saga):
    ...
    async def validate_work_order(self, ctx: SagaContext) -> dict[str, Any]:
        ...
        if not result["valid"]:
            raise SagaStepError(f"Invalid work order: {work_order_id}")
...
async def main():
    ...
    print("=" * 80)
    print("🏭 Manufacturing Production Saga Demo")
    print("=" * 80)
    print("\nThis example demonstrates physical action as a pivot point.")
    print("Once production starts, materials are transformed - no rollback possible.\n")
    
    saga = ManufacturingProductionSaga()
    
    # Scenario 1: Successful production run
    print("\n" + "=" * 80)
    print("🟢 Scenario 1: Successful Production Run")
    print("=" * 80)
    ...
    print(f"\n{'✅' if result.get('saga_id') else '❌'} Production Result:")
    print(f"   Saga ID: {result.get('saga_id')}")
    print(f"   Production Run: {result.get('production_run_id', 'N/A')}")
    ...
    print("\n" + "=" * 80)
    print("🟡 Scenario 2: Pre-Pivot Failure (Material Shortage)")
    print("=" * 80)
    print("Demonstration: If reservation fails, all steps compensate cleanly.\n")
    
    # This would be simulated by making the MES return an error
    print("In this scenario, if material reservation fails:")
    print("  → Work order validation cancelled")
    print("  → No production started (pivot not reached)")
    print("  → System returns to clean state")
    
    # Scenario 3: Post-pivot forward recovery
    print("\n" + "=" * 80)
    print("🔴 Scenario 3: Post-Pivot Failure (Quality Check)")
    print("=" * 80)
    print("""
Quality failures after production starts use forward recovery:

1. RETRY - Attempt rework (grinding, polishing, etc.)
2. RETRY_WITH_ALTERNATE - Use secondary materials for replacement
3. MANUAL_INTERVENTION - Scrap decision by supervisor

This example uses:
- @action("start_production", pivot=True)
- @forward_recovery("run_quality_check")

Key difference:
- Pre-pivot: Cancel work order, release materials, free machine
- Post-pivot: Cannot un-cut steel! Must finish, rework, or scrap
""")
