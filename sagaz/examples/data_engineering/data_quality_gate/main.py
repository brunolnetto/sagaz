...
class DataQualityGateSaga(Saga):
    ...
    async def validate_business_rules(self, ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(f"✅ Business rules passed: {len(rules_applied)} rules, {total_violations:,} minor violations")
...
async def successful_quality_gate_demo():
    ...
    print("\n" + "=" * 80)
    print("🔍 Data Quality Gate Saga - Successful Validation Demo")
    print("=" * 80)
    ...
    print(f"\n{'✅' if result.get('saga_id') else '❌'} Quality Gate Result:")
    print(f"   Saga ID:        {result.get('saga_id')}")
    print(f"   Quality Score:  {result.get('quality_score', 0):.2%}")
    print(f"   Raw Records:    {result.get('raw_record_count', 0):,}")
    print(f"   Loaded Records: {result.get('loaded_record_count', 0):,}")
    print(f"   Null Ratio:     {result.get('null_ratio', 0):.2%}")
    print(f"   Duplicate Ratio:{result.get('duplicate_ratio', 0):.2%}")


async def failed_quality_gate_demo():
    """Demonstrate quality gate failure with quarantine."""
    print("\n" + "=" * 80)
    print("⚠️  Data Quality Gate Saga - Failure with Quarantine Demo")
    print("=" * 80)
    ...
    try:
        ...
        print(f"\n✅ Unexpectedly passed: {result.get('saga_id')}")
        ...
        print("   • No bad data reached production")
...
async def main():
    ...
    print("\n" + "=" * 80)
    print("📚 Data Quality Gate Demo Complete")
    print("=" * 80)
    print("\nKey Benefits of Sagaz for Data Quality:")
    print("  ✅ Multi-stage validation with early termination")
    print("  ✅ Automatic quarantine of bad data")
    print("  ✅ No partial loads to production")
    print("  ✅ Quality metrics tracking")
    print("  ✅ Full audit trail of all checks")
