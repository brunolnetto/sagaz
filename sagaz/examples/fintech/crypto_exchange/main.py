...
class BlockchainSimulator:
    ...
    async def broadcast_transaction(
        ...
        if random.random() < 0.1:  # 10% failure rate
            raise SagaStepError("Failed to broadcast transaction: Network congestion")
...
class CryptoExchangeSaga(Saga):
    ...
    async def validate_trade(self, ctx: SagaContext) -> dict[str, Any]:
        ...
        if not await BlockchainSimulator.validate_wallet(destination_wallet):
            raise SagaStepError(f"Invalid wallet address: {destination_wallet}")
...
async def main():
    ...
    print("=" * 80)
    print("🪙 Cryptocurrency Exchange Saga Demo")
    print("=" * 80)
    print("\nThis example demonstrates blockchain immutability as a pivot point.")
    print("Once a transaction is broadcast, rollback is impossible - only forward recovery.\n")
    
    saga = CryptoExchangeSaga()
    
    # Scenario 1: Successful trade
    print("\n" + "=" * 80)
    print("🟢 Scenario 1: Successful BTC → ETH Exchange")
    print("=" * 80)
    ...
    print(f"\n{'✅' if result.get('saga_id') else '❌'} Trade Result:")
    print(f"   Saga ID: {result.get('saga_id')}")
    print(f"   TX Hash: {result.get('tx_hash', 'N/A')[:30]}...")
    print(f"   Confirmations: {result.get('confirmations', 'N/A')}")
    print(f"   Pivot Reached: {result.get('pivot_reached', False)}")
    
    # Scenario 2: Pre-pivot failure (rollback)
    print("\n" + "=" * 80)
    print("🟡 Scenario 2: Pre-Pivot Failure (Full Rollback)")
    print("=" * 80)
    print("Simulating invalid wallet address - should trigger full compensation...\n")
    ...
    try:
        ...
        print(f"\n❌ Expected failure: {e}")
        print("   → All pre-pivot steps compensated (balance released)")
    
    # Scenario 3: Description of post-pivot behavior
    print("\n" + "=" * 80)
    print("🔴 Scenario 3: Post-Pivot Failure (Forward Recovery)")
    print("=" * 80)
    print("""
Post-pivot failures use forward recovery strategies:

1. RETRY - Check transaction status again
2. RETRY_WITH_ALTERNATE - Send replace-by-fee (RBF) with higher gas  
3. MANUAL_INTERVENTION - Escalate for human resolution

This example uses:
- @action("broadcast_to_blockchain", pivot=True)
- @forward_recovery("wait_confirmations")

Key difference from traditional saga:
- Pre-pivot: Compensate all previous steps (undo)
- Post-pivot: Never compensate - only move forward or pause for intervention
""")
