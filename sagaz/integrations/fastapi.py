...
def create_webhook_router(url_prefix: str = "/webhooks"):
    ...
    async def webhook_handler(source: str, request: Request, background_tasks: BackgroundTasks):
        ...
        # This ensures clients get immediate feedback if required fields are missing
        try:
            ...
            for trigger in triggers:
                ...
                if metadata.idempotency_key:
                    # Check if required key exists in payload
                    key_name = (
                    ...
                    if key_name and isinstance(payload, dict) and key_name not in payload:
                        # Missing required idempotency key - fail fast
        ...
        async def process_event():
            try:
                ...
                # Query storage to get their current status
                from sagaz.core.config import get_config
                ...
                if config.storage and saga_ids:
                    for saga_id in saga_ids:
                        try:
                            ...
                            if state:
                                # Saga already exists - populate its status immediately
                                if "saga_statuses" not in _webhook_tracking[correlation_id]:
                                    _webhook_tracking[correlation_id]["saga_statuses"] = {}
                                ...
                                elif status in (SagaStatus.FAILED, SagaStatus.ROLLED_BACK):
                                    _webhook_tracking[correlation_id]["saga_statuses"][saga_id] = (
                                        "failed"
                                    )
                                    if state.get("error"):
                            ...
                            pass  # saga status will be tracked via listener
